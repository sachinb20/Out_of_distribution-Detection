...
if args.ood_dataset == 'tinyImageNet_crop':
    ...
    ood_dataset = datasets.ImageFolder(root='/home/dell/OOD_Detection/deep_Mahalanobis_detector/data/Imagenet_resize', transform=transform)
    ...
    ood_dataset = datasets.ImageFolder(root='/home/dell/OOD_Detection/deep_Mahalanobis_detector/data/'+ 'LSUN_resize', transform=transform)
...
total = 0
correct = 0
for images,labels in ind_loader:
    ...
    cnn.eval()
    batch_output, _ = cnn(images)
    pred = batch_output.data.max(1)[1]
    equal_flag = pred.eq(labels.data).cpu()
