...
dataset_options = ['cifar10', 'svhn', 'kitti']
...
if args.dataset == 'cifar10':
    ...
    num_classes = 10
    train_dataset = datasets.Kitti(root='data/',
                                  train = True,
                                  transform=train_transform,
                                  download=True)

    test_dataset = datasets.Kitti(root='data/',
                                  train = False,
                                 transform=test_transform,
                                 download=True)
...
train_loader_in = torch.utils.data.DataLoader(dataset=train_dataset,
                                           batch_size=args.batch_size,
                                           shuffle=True,
                                           pin_memory=True,
                                           num_workers=2)

test_loader_in = torch.utils.data.DataLoader(dataset=test_dataset,
                                          batch_size=args.batch_size,
                                          shuffle=False,
                                          pin_memory=True,
                                          num_workers=2)
...
train_loader_bdry = torch.utils.data.DataLoader(bdry_data,
                                        batch_size=32,
                                        shuffle=False,
                                        num_workers=2, 
                                        pin_memory=True)
...
csv_logger = CSVLogger(args=args, filename='logs/' + filename + '.csv',
                       fieldnames=['epoch', 'train_acc', 'test_acc','Joint_confidence_train_loss'])
...
for epoch in range(args.epochs):
    ...
    progress_bar = tqdm(train_loader_in)
    ...
    if (epoch % 10) == 0 and epoch != 0:
        ...
        print('Joint confidence train_loss = '+ str(loss_avg))     

    test_acc, conf_min, conf_max, conf_avg = test(test_loader_in)
    tqdm.write('test_acc: %.3f, conf_min: %.3f, conf_max: %.3f, conf_avg: %.3f' % (test_acc, conf_min, conf_max, conf_avg))

    scheduler.step(epoch)

    row = {'epoch': str(epoch), 'train_acc': str(accuracy), 'test_acc': str(test_acc), 'Joint_confidence_train_loss': str(loss_avg)}
